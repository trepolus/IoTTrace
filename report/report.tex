...
\section{Conclusion}
As described in this paper, we got a good overview of how taxis in Shanghai behave in what time of day. We compared different heatmaps which are the visual representation of the taxis' density. This representation refers to the amount of taxis on a particular area of the city center. The warmer the color, the more taxis in the same area.
\\
We mentioned in the section about observable patterns that it is possible to find several different behaviours based on the different hours ranges, but it's not the most important point, because we can combine time of day parameter with the taxi occupied condition, if taxi is with or without passengers.
\\
